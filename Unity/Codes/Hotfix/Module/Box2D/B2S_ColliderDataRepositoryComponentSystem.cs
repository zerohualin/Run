namespace ET
{
    ...
    public static class B2S_ColliderDataRepositoryComponentSystem
    {
        ...
        public static void ReadcolliderData(this B2S_ColliderDataRepositoryComponent self)
        {
            ...
            // byte[] mfile0 = XAssetLoader.LoadAsset<TextAsset>(XAssetPathUtilities.GetB2SColliderConfigPath(this.colliderDataName[0]))?.bytes;
            // //这里不进行长度判断会报错，正在试图访问一个已经关闭的流，咱也不懂，咱也不敢问
            // if (mfile0 != null && mfile0.Length > 0)
            // {
            //     self.BoxColliderDatas = BsonSerializer.Deserialize<ColliderDataSupporter>(mfile0);
            //     Log.Info($"已经读取矩形数据，数据大小为{mfile0.Length}");
            // }
            //
            ...
            // {
            //     self.CircleColliderDatas = BsonSerializer.Deserialize<ColliderDataSupporter>(mfile1);
            //     Log.Info($"已经读取圆形数据，数据大小为{mfile1.Length}");
            // }
            //
            // byte[] mfile2 = XAssetLoader.LoadAsset<TextAsset>(XAssetPathUtilities.GetB2SColliderConfigPath(this.colliderDataName[2]))?.bytes;
            // if (mfile2 != null && mfile2.Length > 0)
            // {
            //     self.PolygonColliderDatas = BsonSerializer.Deserialize<ColliderDataSupporter>(mfile2);
            //     Log.Info($"已经读取多边形数据，数据大小为{mfile2.Length}");
            // }
#endif
        }
