...
namespace ET
{
    ...
    [ObjectSystem]
    public class B2S_ColliderComponentAwakeSystem : AwakeSystem<B2S_ColliderComponent, CreateSkillColliderArgs>
    {
        public override void Awake(B2S_ColliderComponent self, CreateSkillColliderArgs createSkillColliderArgs)
        {
            B2SCollisionRelationConfig serverB2SCollisionRelationConfig = B2SCollisionRelationConfigCategory.Instance
    ...
    [FriendClass(typeof(Unit))]
    [ObjectSystem]
    public class B2S_ColliderComponentAwakeSystem1 : AwakeSystem<B2S_ColliderComponent, CreateHeroColliderArgs>
    {
        public override void Awake(B2S_ColliderComponent self, CreateHeroColliderArgs args)
        {
            self.WorldComponent = self.GetParent<Unit>().BelongToRoom.GetComponent<B2S_WorldComponent>();
            ...
            B2S_ColliderDataLoadHelper.ApplyFixture(self.B2S_ColliderDataStructureBase, self.Body, self.GetParent<Unit>());
            self.SyncBody();
        }
    }

    public class B2S_HeroColliderDataFixedUpdateSystem : LateUpdateSystem<B2S_ColliderComponent>
    {
        public override void LateUpdate(B2S_ColliderComponent self)
        {
...
#if !SERVER
                Game.EventSystem.PublishAsync(new DebugVisualBox2D() {Unit = unit}).Coroutine();
#endif
            }
        }
    }
