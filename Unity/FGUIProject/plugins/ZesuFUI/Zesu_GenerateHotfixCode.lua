...
local function generateClass()
    ...
    writer:startBlock()
        if string.match(classInfo.className, "_Component") then
            local enumName = string.gsub(classInfo.className, "_Component", "")
            enumName = string.gsub(enumName, "FUI_", "")
            writer:writeln('[FGUIComponent(Cfg.FGUIType.%s)]', enumName)
        end
        writer:writeln('public sealed class %s : Entity, IFGUIComponent, IAwake<GObject>', classInfo.className)
        writer:startBlock()
