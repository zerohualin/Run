...
namespace ET
{
    ...
    public class B2S_BoxColliderVisualHelper : B2S_ColliderVisualHelperBase
    {
        ...
        public override void SavecolliderData()
        {
            ...
            string Path = $"{B2S_BattleColliderExportPathDefine.ServerColliderDataSavePath}/{this.ColliderDataFileName}.bytes";
            using (FileStream file = File.Create(Path))
            {
                BsonSerializer.Serialize(new BsonBinaryWriter(file), this.MColliderDataSupporter);
            }
            byte[] fileBytes = File.ReadAllBytes(Path);
            ColliderDataSupporter colliderData = BsonSerializer.Deserialize<ColliderDataSupporter>(fileBytes);
            Log.Error(colliderData.ToString());
        }
