...
set GEN_CLIENT=dotnet %WORKSPACE%\Tools\Luban\Tools\Luban.ClientServer\Luban.ClientServer.dll
...
%GEN_CLIENT% -j cfg --^
 ...
 --output_code_dir %WORKSPACE%\Server\Model\Generate\Luban ^
...
%GEN_CLIENT% -j cfg --^
 -d %CONF_ROOT%\Defines\__root__.xml ^
 --input_data_dir %CONF_ROOT%\Datas ^
 --output_code_dir %WORKSPACE%\Unity\Codes\Model\Generate\Luban ^
